...
Run the script by clicking the *Source* button. If you see an error, e.g., 

    Error in library("tidyverse") : there is no package called 'tidyverse'

then you have to install the package as explained in [Install an R package](#install-an-r-package) 
...
The goal of this 50-minute tutorial is to introduce the R novice to basic data carpentry skills:

```{r echo = FALSE}
df6 <- wrapr::build_frame(
        "filename"            , "item" |
        "data-basics-01.png"  , "Importing data" |
        "data-basics-02.png"  , "Reading data into R; writing data from R to file." |
        "data-basics-03.png"  , "Identifying data structure, the number of observations, the number of variables, and their types." |
        "data-basics-04.png"  , "Transforming, reshaping, and joining data to obtain the desired form for your intended graph or analysis. "
        ) %>%
...
- the R data structure, e.g., vector, matrix, data frame, time series, list 
- the number of observations 
- the number of variables 
- the name and type or class of each variable 
...
### Case 1: Adjusting for inflation 

This example illustrates many of the basic skills outlined above.  

Over any time period with inflation, a dollar buys less at the end the period than it did at the beginning of the period. Thus, in 1973 a single 20-year old  could live comfortably on \$5/hour but in 2018 (45 years later) that same 20-year-old has to earn \$`r 5 * round(251/44.4, 0)`/hour to achieve the same modest standard of living. 
...
The URL to obtain the CPI data from the FRED is [http://research.stlouisfed.org/fred2/data/CPIAUCNS.txt](http://research.stlouisfed.org/fred2/data/CPIAUCNS.txt). If you click on the link, you can see that the data starts on line14; the first 13 lines of the text are meta-information:
...
- Import the data, skipping the first 13 lines 

```{r}
x <- rio::import(cpi_url, skip = 13)
```

- Write to file and convert to CSV

```{r}
rio::export(x, "data/cip.csv")
```

- Read the CSV file  

```{r}
cpi <- read_csv("data/cip.csv")
```
