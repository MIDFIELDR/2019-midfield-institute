...
- [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.html){target="_blank"} order tibble rows by an expression  
- [`filter()`](https://dplyr.tidyverse.org/reference/filter.html){target="_blank"} choose rows/cases where conditions are true  
- [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html){target="_blank"}  adds new variables and preserves existing ones  
- [`select()`](https://dplyr.tidyverse.org/reference/select.html){target="_blank"} keeps only the variables you mention   
- [`rename()`](https://dplyr.tidyverse.org/reference/select.html){target="_blank"} a variant of `select()` keeps all variables   
- [`left_join()`](https://dplyr.tidyverse.org/reference/join.html){target="_blank"} join two data frames   

rio 

- [`import()`](https://cran.r-project.org/web/packages/rio/vignettes/rio.html){target="_blank"} read in a data frame from a file 
- [`export()`](https://cran.r-project.org/web/packages/rio/vignettes/rio.html){target="_blank"} write a data frame to a file 


inspectdf 

- [`inspect_mem()`](https://github.com/alastairrushworth/inspectdf){target="_blank"} summarize memory usage in a data frame  
- [`inspect_na()`](https://github.com/alastairrushworth/inspectdf){target="_blank"} summarize the rate of missingness in a data frame    
- [`inspect_ cat()`](https://github.com/alastairrushworth/inspectdf){target="_blank"} summarize the levels within each categorical variable 

lubridate 

- [`year()`](https://lubridate.tidyverse.org/reference/year.html){target="_blank"} return year from a date object 

seplyr 

- [`group_summarize()`](https://winvector.github.io/seplyr/reference/group_summarize.html){target="_blank"} group a data frame by the grouping variables and compute user summaries 

base R 

- [`(head)`](http://rfunction.com/archives/699){target="_blank"} returns the last part of an R object 
- [`tail()`](http://rfunction.com/archives/699){target="_blank"} returns the last part of an R object 
- [`mean()`](https://www.rdocumentation.org/packages/base/versions/3.6.0/topics/mean){target="_blank"} determine an arithmetic mean 
